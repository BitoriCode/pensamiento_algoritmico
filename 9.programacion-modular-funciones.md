...
- [Funciones en Programación (Python)](#funciones-en-programación-python)
    - [Tabla de contenidos](#tabla-de-contenidos)
  - [Introducción a las funciones](#introducción-a-las-funciones)
  - [Reglas para definir funciones en Python](#reglas-para-definir-funciones-en-python)
    - [Reglas de sintaxis (obligatorias)](#reglas-de-sintaxis-obligatorias)
    - [Convenciones de estilo (recomendadas, PEP 8)](#convenciones-de-estilo-recomendadas-pep-8)
    - [Errores comunes al definir funciones](#errores-comunes-al-definir-funciones)
  - [Definición y llamada de funciones](#definición-y-llamada-de-funciones)
  ...
  - [Recursividad (Funciones recursivas)](#recursividad-funciones-recursivas)
  - [Funciones anónimas (expresiones *lambda*)](#funciones-anónimas-expresiones-lambda)
  - [Manejo de errores y excepciones en funciones](#manejo-de-errores-y-excepciones-en-funciones)
  - [Buenas prácticas al escribir funciones](#buenas-prácticas-al-escribir-funciones)
  - [¿Y después de las funciones? Métodos y POO](#y-después-de-las-funciones-métodos-y-poo)
  - [Conclusión](#conclusión)

> Las secciones marcadas con ⚙️ son **temas avanzados/opcionales** para profundización. No son requisito para los ejercicios básicos del curso.
...
---
## Reglas para definir funciones en Python

Antes de ver ejemplos, es importante conocer las **reglas de sintaxis** que Python exige para definir y usar funciones correctamente. Tener estas reglas claras desde el inicio evitará errores comunes.

### Reglas de sintaxis (obligatorias)

1. **Usar la palabra reservada `def`**: Toda función se define comenzando con `def`, seguida del nombre y paréntesis.
2. **Nombre válido de función**:
   - Debe comenzar con una **letra** (a-z, A-Z) o guion bajo (`_`).
   - Puede contener letras, números y guiones bajos, pero **no espacios ni caracteres especiales** (`@`, `#`, `-`, etc.).
   - **No puede ser una palabra reservada** de Python (`if`, `for`, `while`, `return`, `class`, `def`, etc.).
   - Python distingue mayúsculas de minúsculas: `Calcular` y `calcular` son nombres diferentes.
3. **Paréntesis obligatorios**: Después del nombre siempre van paréntesis `()`, estén vacíos o con parámetros.
4. **Dos puntos al final del encabezado**: La línea `def nombre():` debe terminar con `:`.
5. **Cuerpo indentado**: Todo el código dentro de la función debe estar indentado (generalmente con 4 espacios). Python usa la indentación para saber qué pertenece a la función.
6. **Definir antes de llamar**: La función debe estar definida **antes** de la línea donde se invoca. Python lee el código de arriba hacia abajo.
7. **Parámetros con valores por defecto van al final**: Si mezclas parámetros obligatorios y opcionales, los que tienen valor por defecto deben ir después de los obligatorios.
...
# ✅ Correcto (parámetro opcional al final)
def saludar(nombre, idioma="es"):
    ...

# ❌ Error: nombre con espacio
def calcular area():
    ...

# ❌ Error: falta los dos puntos
def calcular_area()
    return 0

# ❌ Error: parámetro por defecto antes del obligatorio
def saludar(idioma="es", nombre):
    ...
```

### Convenciones de estilo (recomendadas, PEP 8)

Estas no son obligatorias para que el código funcione, pero son el estándar en la comunidad Python:

| Convención | Ejemplo correcto | Ejemplo a evitar |
|---|---|---|
| Nombres en **snake_case** (minúsculas con `_`) | `calcular_promedio` | `CalcularPromedio`, `calcularpromedio` |
| Nombres descriptivos (verbos) | `obtener_maximo`, `validar_edad` | `func1`, `f`, `cosa` |
| Incluir **docstring** | `"""Calcula el área."""` | Sin documentación |
| Máximo ~50 líneas por función | Dividir en subfunciones | Función de 200 líneas |

### Errores comunes al definir funciones

| Error | Qué pasa | Ejemplo |
|---|---|---|
| Olvidar los `:` | `SyntaxError` | `def suma()` |
| No indentar el cuerpo | `IndentationError` | Cuerpo al mismo nivel que `def` |
| Llamar antes de definir | `NameError` | Usar `suma()` antes de `def suma():` |
| Olvidar `return` | La función retorna `None` | Querer usar el resultado y obtener `None` |
| Olvidar los `()` en la llamada | No se ejecuta la función | `resultado = suma` en vez de `resultado = suma()` |

> **Resumen de la sección:**
> - Toda función se define con `def nombre(parametros):` y cuerpo indentado.
> - Los nombres deben ser válidos (sin espacios, sin palabras reservadas) y descriptivos en snake_case.
> - Siempre definir la función **antes** de llamarla.
> - Los parámetros opcionales (con valor por defecto) van **después** de los obligatorios.
> - Los errores más comunes son: olvidar `:`, no indentar, olvidar `return` y olvidar `()` al llamar.
...
```
Hola, Mariana! Bienvenido al programa.
...
```
┌─────────────────────────────────────────────────┐
│  def nombre_funcion(parametros):   ← Encabezado │
│      """Docstring"""               ← Documentación│
│      instrucciones                 ← Cuerpo     │
│      return valor                  ← Retorno    │
└─────────────────────────────────────────────────┘

  resultado = nombre_funcion(argumentos)  ← Llamada
```
...
print(contar_caracter(texto, 'r'))  # Debería imprimir 2.
...
> **Resumen de la sección:**
> - Una función se **define** con `def nombre(parametros):` y se **llama** con `nombre(argumentos)`.
> - La definición no ejecuta el código; solo la llamada lo hace.
> - Las funciones permiten **reutilizar** código y hacer los programas más legibles.
> - La definición debe aparecer **antes** de la primera llamada en el código.
...
> **Resumen de la sección:**
> - **Parámetros** = nombres en la definición; **argumentos** = valores en la llamada.
> - Se pueden pasar argumentos por **posición** o por **nombre** (`clave=valor`).
> - Los **valores por defecto** hacen que un parámetro sea opcional.
> - Objetos **mutables** (listas, diccionarios) pueden modificarse dentro de la función y el cambio se refleja fuera.
> - Nunca uses objetos mutables como valores por defecto (usa `None` en su lugar).
...
> **Resumen de la sección:**
> - `return` devuelve un valor al llamador y **termina** la ejecución de la función.
> - Si no hay `return`, la función devuelve `None` automáticamente.
> - **Imprimir ≠ retornar**: `print()` muestra en consola, `return` entrega el valor al código que llamó.
> - Se pueden retornar **múltiples valores** separados por coma (se empaquetan en una tupla).
...
> **Resumen de la sección:**
> - Variables dentro de funciones son **locales** (solo existen durante la ejecución de la función).
> - Variables fuera de funciones son **globales** (accesibles en todo el módulo).
> - Para **leer** una global dentro de una función, basta con usarla. Para **modificarla**, necesitas `global`.
> - **Buena práctica**: evitar modificar globales; preferir pasar datos como parámetros y retornar resultados.
...
> ⚙️ **Tema avanzado/opcional**: Esta sección profundiza en un concepto que no es necesario dominar para los ejercicios básicos del curso, pero es muy valioso para desarrollar el pensamiento algorítmico.
...
En términos más simples: una función recursiva es como una matrioska (las muñecas rusas) o un espejo frente a otro espejo (efecto infinito): la definición de la función incluye una versión más pequeña de sí misma. Para que esto no se ejecute infinitamente, **debe haber al menos un caso base que rompa la recursión**. Toda función recursiva tiene dos partes importantes:
...
> **Resumen de la sección:**
> - Una función **recursiva** se llama a sí misma para resolver subproblemas más pequeños.
> - Toda recursión necesita un **caso base** (condición de parada) y un **caso recursivo**.
> - Python tiene un límite de ~1000 llamadas recursivas de profundidad.
> - Toda recursión se puede reescribir como iteración (bucle), y viceversa.
...
> ⚙️ **Tema avanzado/opcional**: Las lambdas son útiles para código más conciso, pero no son indispensables en un primer curso. Puedes volver a esta sección más adelante.
...
> **Resumen de la sección:**
> - `lambda parametros: expresion` crea una función anónima de una sola expresión.
> - Se usan principalmente como argumento de funciones como `sorted()`, `map()`, `filter()`.
> - Si la lógica es compleja, usa `def` con nombre. Lambda es solo para casos simples y puntuales.
...
> **Resumen de la sección:**
> - Usa `raise` para lanzar excepciones cuando detectes datos inválidos.
> - Usa `try/except` para capturar excepciones y evitar que el programa se detenga.
> - Sé **específico** en las excepciones que capturas (evita `except Exception: pass`).
...
> **Resumen de la sección:**
> - Cada función debe tener **una sola responsabilidad** (hacer una cosa bien).
> - Usa **nombres descriptivos** en snake_case y agrega **docstrings**.
> - Evita efectos secundarios: prefiere retornar valores nuevos en lugar de modificar datos externos.
> - Valida las entradas de la función y proporciona mensajes de error claros.
> - Antes de escribir tu propia función, verifica si Python ya ofrece algo similar (`len`, `sorted`, `sum`, etc.).
...
## ¿Y después de las funciones? Métodos y POO

> Las funciones que acabamos de estudiar son la base de un concepto más amplio: los **métodos** en Programación Orientada a Objetos (POO). Un método es simplemente una función definida dentro de una **clase**, que opera sobre los datos de un objeto.
>
> Este tema se desarrolla en detalle en las notas complementarias:  
> **[10.introducción a POO y métodos](10.introduccion_poo_metodos.md)**
>
> Se recomienda estudiar primero todos los conceptos de funciones de este documento antes de pasar a POO.

---

## Conclusión

En estas notas de clase hemos recorrido los conceptos fundamentales de las funciones en Python:

- **Definición y llamada** de funciones con `def`.
- **Parámetros y argumentos**: posicionales, nombrados y con valores por defecto.
- **Valores de retorno** con `return` (incluyendo retornos múltiples).
- **Ámbito de variables**: locales vs globales.
- **Recursividad**: funciones que se llaman a sí mismas (tema avanzado).
- **Funciones lambda**: funciones anónimas de una expresión (tema avanzado).
- **Manejo de errores**: `try/except/finally` y `raise`.
- **Buenas prácticas**: nombres claros, responsabilidad única, documentación, evitar efectos secundarios.

Como estudiante de primer semestre, es normal sentirse abrumado al principio con tantos detalles. **Recomendación**: práctica constante. Toma cada sección y experimenta escribiendo tus propios ejemplos, modificando los que se mostraron, y resolviendo los ejercicios propuestos. Prueba a escribir pequeños programas en Visual Studio Code, ejecutarlos, depurarlos (VS Code tiene un depurador que puedes usar para hacer seguimiento paso a paso, muy útil para entender recursión o flujo de llamadas). 

No dudes en introducir errores adrede para ver cómo se comportan (por ejemplo, llama a una función antes de definirla para ver el error, o provoca una excepción de tipo para ver el mensaje). Cada error es una oportunidad de aprendizaje para entender mejor cómo funciona Python.

Conforme avances, verás que las funciones se vuelven la piedra angular de programas más complejos. Estas notas pueden servirte de referencia rápida más adelante; incluso podrías ampliarlas con tus propias observaciones o ejemplos descubiertos. Cuando estés listo, continúa con las [notas de POO y métodos](9.%20introduccion_poo_metodos.md) para ver cómo las funciones se integran en la programación orientada a objetos.
